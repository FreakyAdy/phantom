...
**v2 Target Status**: **NOT ACHIEVED — real spec-decode measurement pending (ADR-022/023; no fabricated figures shipped)**
...
|---|---|---|---|---|
| **PHANTOM v2 Speculative Decode (carried mech.)** | llama.cpp native draft-model decode toward 7-10 / 14-18 / 14 tok/s | No real measurement yet (`SKIPPED_LLAMA_CPP_NOT_INSTALLED` locally); model-free EAGLE harness DELETED | **IN PROGRESS — REAL PATH ONLY** (ADR-022/023) | [`benchmarks/run_real.py`](benchmarks/run_real.py) `--focus spec-decode` |
| **EAGLE-3 Feature-Fusion Heads** | Layers [0,30,60,79] fusion, K=5 token heads | ~3M params; trained only on LCG synthetic data (loss ~423) | **SIMULATION — DEFERRED** (ADR-022) | [`tests/unit/test_eagle_heads.py`](tests/unit/test_eagle_heads.py) |
| **Fused Attention + FFN Kernels** | PyTorch fused path with Triton dispatch | Triton branches are PyTorch-fallback stubs | **STUB — NOT VERIFIED** | [`python/phantom/kernels/dispatch.py`](python/phantom/kernels/dispatch.py) |
| **Wraith v2 Prefetch Scheduler** | Confidence-gated async layer staging | LSTM predictor replaced; OS-level fadvise/madvise hints only | **IN PROGRESS** | [`tests/unit/test_wraith_prefetch.py`](tests/unit/test_wraith_prefetch.py) |
| **Heterogeneous Speculative Runtime** | GPU Draft + CPU Batched GEMM Target Verifier | Unit-level consistency only; no real draft/target pair measured | **SIMULATION ZONE** (ADR-022) | [`tests/unit/test_speculative_engine.py`](tests/unit/test_speculative_engine.py) |
...
| **Colab v2 Test Harness** | Real native spec-decode sweep on T4 (`--ngl` × `--n-batch`) | Notebook Step 5 rewired to `run_real.py --focus spec-decode`; dry-run/live-failure return NO fabricated numbers | **IN PROGRESS** | [`scripts/colab_v2_runner.py`](scripts/colab_v2_runner.py) |
| **MoE Prefetch Correlation** | Wraith layer predictions vs expert activations | Prior 0.10–0.15 overlap/stats produced by deleted model-free harness | **VOIDED (ADR-023)** | [`python/phantom/prefetch/moe_correlation.py`](python/phantom/prefetch/moe_correlation.py) |
...
| **`Qwen3-30B-A3B`** | **30.5 Billion** | **MoE (3.3B Active)** | **RTX 4050 (6GB VRAM)** | **4.66 GB VRAM + 11.32 GB RAM** | **12.95 tok/s (Local); Cloud figure removed (ADR-023)** | **PASS — 100% Verified (local)** ([`test_03`](file:///c:/Work/Projects/Solution%20is%20all%20You%20need/docs/testing/test_03_qwen3_30b_a3b.md)) |
...
Milestone 2.0: PHANTOM v2 MD Blueprint (torch EAGLE-3 stack) — BUILD COMPLETE / PERFORMANCE UNVALIDATED
[██████████████████████████████████░░░░░░]  85% BUILD COMPLETE — v2 tok/s targets NOT MET (ADR-022 supersedes)

Milestone 2.1: Ground Truth Remediation (Phases 0–7)
[████████████████████████████████████████] 100% COMPLETED (2026-09-15)

Milestone 2.2: Physics-Honest v2 — Real Spec-Decode Path (Phases 0–1)
[██████████████████████████░░░░░░░░░░░░░░]  60% IN PROGRESS — purge done, live Colab measurement pending
```
...
- [x] Ran live benchmark of `Qwen3-30B-A3B` on Local Laptop (12.95 tok/s). Colab 24.79 tok/s figure removed per ADR-023 (dry-run artifact).
- [x] Expert routing stability (46.91 μs router latency) and 9.93× FLOP reduction (architecture-level, from published arch + local run).
...
#### Milestone 2.0 — PHANTOM v2 MD Blueprint Implementation (BUILD COMPLETE — PERFORMANCE UNVALIDATED, ADR-022)
- [x] Canonical spec (`docs/specs/PHANTOM_V2_SPEC.md`) and ADR-016 superseding ADR-015 purge.
- [x] EAGLE-3 heads + training pipeline (`eagle_heads.py`, `eagle_train.py`) — SIMULATION ONLY (LCG synthetic training).
- [x] Kernel fusion rebuild (`kernels/attention/`, `kernels/ffn/`, `python/phantom/kernels/dispatch.py`) — Triton branches are stubs.
- [x] Wraith v2 prefetch scheduler integrated with speculative engine.
- [x] Selective Q3 quantization and 40% adaptive sparsity gates — SIMULATION flags.
- [x] Live model loader + CLI v2 flags (`--spec-mode`, `--eagle-heads`, ablation flags).
- [x] ~~v2 benchmark suite~~ **DELETED (ADR-023)**: model-free harness + `v2_latest.json` removed; replaced by `run_real.py --focus spec-decode`.
- [ ] Quality gates on REAL numbers: spec-decode tok/s rows in `latest.json` (pending Colab live run).

#### Milestone 2.2 — Physics-Honest v2: Real Spec-Decode Path (IN PROGRESS — Phases 0–1 COMPLETE)
- [x] Phase 0: Fixed `cmd_run` AttributeError so the real llama.cpp path executes; llama-cpp draft metrics + graceful `SKIPPED_LLAMA_CPP_NOT_INSTALLED`; `run_real.py --focus spec-decode` + `--ngl`/`--n-batch` sweeps; `test_runtime_metrics.py`; 43 unit tests PASS + parity PASS.
- [x] Phase 1: Deleted model-free harness + fabricated `v2_latest.json`; `colab_v2_runner.py` fabrication purged; `benchmark --v2` rewired to real subprocess; `RESULTS.md` section 6 honest; `test_16` VOIDED; `test_03` 24.79 removed; README Ollama comparison + DRY-RUN SIMULATIONS banner; ADR-022/023; notebook Step 1/5 updated.
- [ ] Phase 2: Commit+push, live Colab spec-decode run (`qwen2.5-coder-32b`, draft `qwen2.5-0.5b`, `--ngl` × `--n-batch` sweeps), export `latest.json`.
- [ ] Phase 3: Ingest results; close gap to targets (levers: `--no-mmap`, Q3_K_S, stronger drafts, batch tuning).
- [ ] Phase 4: Declare achievement ONLY with real `spec_decode` rows in `latest.json`; update RESULTS.md/README atomically.
...
- [x] Determinetic: Dense 32B → 14 tok/s is physically impossible (DDR5 bandwidth wall). MoE 30B and dense 14B remain the target tiers (v2 speed targets NOT yet met on real hardware).
...
- [x] Expert routing correlation analysis for MoE prefetching — VOIDED (ADR-023; model-free output).
- [x] Colab v2 harness (`scripts/colab_v2_runner.py`) — fabrication purged; notebook Step 5 rewired to real spec-decode.
- [ ] Colab live-weight spec-decode on T4 (`qwen2.5-coder-32b`, draft `qwen2.5-0.5b`, `--ngl`/`--n-batch` sweeps) — Phase 2.
- [x] Evaluate llama.cpp backend integration (done — ADR-017/022: native draft-model spec-decode is the carried v2 mechanism).
